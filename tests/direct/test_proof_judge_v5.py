...
def test_create_job_escrows_gen_indexes_and_audits(
    ...
    assert job.policy_version == "PJ_V5_COMPOSABLE_MILESTONES"
...
def create_project_milestone(direct_vm, contract, sponsor, contractor, project_id, job_id, prerequisite="", reward=3000, deadline_offset=3600):
    direct_vm.sender = sponsor
    direct_vm.value = reward
    contract.create_milestone(project_id, job_id, prerequisite, "0x" + contractor.hex(), REQUIREMENT, RUBRIC, future_deadline(deadline_offset))
    direct_vm.value = 0
    direct_vm.deal(contract.address, 100000)


def test_v5_standalone_job_is_project_indexed(direct_vm, direct_deploy, direct_alice, direct_bob):
    contract = direct_deploy("contracts/proof_judge_v5.py")
    create_demo_job(direct_vm, contract, direct_alice, direct_bob)
    job = contract.get_job("milestone-1")
    assert job.project_id == "milestone-1"
    assert job.prerequisite_job_id == ""
    assert int(job.milestone_index) == 0
    assert int(contract.get_project_job_count("milestone-1")) == 1
    assert contract.is_milestone_unlocked("milestone-1") is True


def test_v5_second_stage_is_locked_until_first_is_paid(direct_vm, direct_deploy, direct_alice, direct_bob):
    contract = direct_deploy("contracts/proof_judge_v5.py")
    create_project_milestone(direct_vm, contract, direct_alice, direct_bob, "product-v5", "phase-1", "", 2000, 3600)
    create_project_milestone(direct_vm, contract, direct_alice, direct_bob, "product-v5", "phase-2", "phase-1", 3000, 7200)
    assert contract.get_project_job_id("product-v5", 0) == "phase-1"
    assert contract.get_project_job_id("product-v5", 1) == "phase-2"
    assert contract.is_milestone_unlocked("phase-2") is False
    direct_vm.sender = direct_bob
    with direct_vm.expect_revert("Prerequisite milestone must be PAID"):
        contract.submit_evidence("phase-2", "https://example.com", "https://www.iana.org/help/example-domains")

    submit_demo_evidence(direct_vm, contract, direct_bob, "phase-1")
    mock_approved(direct_vm)
    contract.resolve_job("phase-1")
    direct_vm.sender = direct_bob
    contract.claim_payment("phase-1")
    assert contract.is_milestone_unlocked("phase-2") is True


def test_v5_progress_and_participant_stats(direct_vm, direct_deploy, direct_alice, direct_bob):
    contract = direct_deploy("contracts/proof_judge_v5.py")
    create_project_milestone(direct_vm, contract, direct_alice, direct_bob, "sdk-rollout", "sdk-core", "", 2000, 3600)
    create_project_milestone(direct_vm, contract, direct_alice, direct_bob, "sdk-rollout", "sdk-docs", "sdk-core", 3000, 7200)
    submit_demo_evidence(direct_vm, contract, direct_bob, "sdk-core")
    mock_approved(direct_vm)
    contract.resolve_job("sdk-core")
    direct_vm.sender = direct_bob
    contract.claim_payment("sdk-core")

    progress = contract.get_project_progress("sdk-rollout")
    assert int(progress.total_milestones) == 2
    assert int(progress.paid_milestones) == 1
    assert int(progress.open_milestones) == 1
    assert int(progress.total_reward) == 5000
    assert int(progress.settled_reward) == 2000

    sponsor_stats = contract.get_participant_stats("0x" + direct_alice.hex())
    contractor_stats = contract.get_participant_stats("0x" + direct_bob.hex())
    assert int(sponsor_stats.sponsored_jobs) == 2
    assert int(contractor_stats.assigned_jobs) == 2
    assert int(contractor_stats.paid_jobs) == 1
    assert int(contractor_stats.total_earned) == 2000


def test_v5_chain_rejects_wrong_prerequisite_or_contractor(direct_vm, direct_deploy, direct_alice, direct_bob, direct_charlie):
    contract = direct_deploy("contracts/proof_judge_v5.py")
    create_project_milestone(direct_vm, contract, direct_alice, direct_bob, "guarded-chain", "first", "", 2000, 3600)
    direct_vm.sender = direct_alice
    direct_vm.value = 2000
    with direct_vm.expect_revert("previous project milestone"):
        contract.create_milestone("guarded-chain", "wrong-link", "", "0x" + direct_bob.hex(), REQUIREMENT, RUBRIC, future_deadline(7200))
    with direct_vm.expect_revert("same contractor"):
        contract.create_milestone("guarded-chain", "wrong-contractor", "first", "0x" + direct_charlie.hex(), REQUIREMENT, RUBRIC, future_deadline(7200))


def test_v5_chain_requires_increasing_deadlines(direct_vm, direct_deploy, direct_alice, direct_bob):
    contract = direct_deploy("contracts/proof_judge_v5.py")
    first_deadline = future_deadline(7200)
    direct_vm.sender = direct_alice
    direct_vm.value = 2000
    contract.create_milestone("deadline-chain", "first", "", "0x" + direct_bob.hex(), REQUIREMENT, RUBRIC, first_deadline)
    with direct_vm.expect_revert("deadlines must increase"):
        contract.create_milestone("deadline-chain", "second", "first", "0x" + direct_bob.hex(), REQUIREMENT, RUBRIC, first_deadline)


def test_v5_refund_updates_settlement_stats(direct_vm, direct_deploy, direct_alice, direct_bob):
    contract = direct_deploy("contracts/proof_judge_v5.py")
    create_demo_job(direct_vm, contract, direct_alice, direct_bob, "expired-v5")
    direct_vm.warp((datetime.now(timezone.utc) + timedelta(days=2)).isoformat())
    direct_vm.sender = direct_alice
    contract.refund_expired("expired-v5")
    sponsor_stats = contract.get_participant_stats("0x" + direct_alice.hex())
    contractor_stats = contract.get_participant_stats("0x" + direct_bob.hex())
    assert int(sponsor_stats.total_recovered) == 2500
    assert int(contractor_stats.refunded_jobs) == 1
